...
static int sendPacket(MQTTClient* c, int length, Timer* timer)
{
    ...
    //@sz if the timer is expired then return without error
    if(TimerIsExpired(timer))
      return TIMEOUT;
      
    while (sent < length)//@sz
    {
        ...
        case PUBLISH:
        {
            ...
            if (msg.qos != QOS0)
            {
                ...
                else
                {
                    ...
                    Timer timer_s;
                    TimerInit(&timer_s);
                    TimerCountdownMS(&timer_s, 1000);
                    //@sz ,end
                    rc = sendPacket(c, len, &timer_s);
                }
                if (rc == FAILURE)
                {
                    goto exit; // there was a problem
                }
                else if(rc == TIMEOUT) //@sz ignore timeout error
                {
                  rc =  MQSUCCESS;
                  goto exit;
                }
